...
For the Earth, quantities are measured that remain model assumptions for other planets: the
gravity field from satellite orbits, the structure from free oscillations and seismic travel
times, the tidal evolution from laser ranging to the Moon. This text gives the parameters with
their uncertainties, the open disputes with both sides, and what Orrery reproduces of them.
...
|---|---|---|---|---|
| $GM_\oplus$ (TCG) | $3.986004418 \cdot 10^{14}\,\mathrm{m}^3\,\mathrm{s}^{-2}$ | $8 \cdot 10^{5}\,\mathrm{m}^3\,\mathrm{s}^{-2}$ | satellite orbits, mainly LAGEOS; including the atmosphere | [Petit and Luzum 2010](literatur:petit-2010); [Ries et al. 1992](literatur:ries-1992) |
...
and hence $G$; Chen et al. give $A$, $B$ and $C$ each to
$9.6 \cdot 10^{33}\,\mathrm{kg}\,\mathrm{m}^2$, limited by $G$ and the dynamical ellipticity
([Chen et al. 2015](literatur:chen-2015)). For the mean moment of inertia
$I = (A + B + C)/3$,
...
travel-time observations, 100 quality factors, mass and moment of inertia, plus 1.75 million
P- and S-wave travel times from ISC data; the outer 220 km of the mantle had to be made
...
$I/(M R^2) = 0.3308$. The inner core extends to a radius of 1,221.5 km, the core–mantle
boundary lies at 3,480.0 km, i.e. 2,891 km deep; the mantle transition zone lies between
400 and 670 km depth. The density polynomials give 13.09 g/cm³ at the centre, a jump from 12.17
to 12.76 g/cm³ at the inner core and from 5.57 to 9.90 g/cm³ at the core–mantle boundary.
...
The topographic range is 20.4 km; at $1.4 \cdot 10^{21}\,\mathrm{kg}$ the hydrosphere has
about 275 times the mass of the atmosphere ([NSSDC Earth Fact Sheet](quelle:nssdc-earth)). The
Earth is the only known planet with plate tectonics; the principal driving force is the sinking
of cold, dense lithosphere in subduction zones ([Stern 2005](literatur:stern-2005)). When plate
tectonics began is open; proposals range from the Hadean to the Neoproterozoic. Palin et al.
see in metamorphic rocks and geodynamic models a global onset no later than about 3 billion
years ago and interpret older subduction traces as local, plume-induced subduction
([Palin et al. 2020](literatur:palin-2020)). Stern concludes from the first appearance of
ophiolites, blueschists and ultrahigh-pressure rocks that subduction in the modern style began
only in the Neoproterozoic ([Stern 2005](literatur:stern-2005)).

## Atmosphere and magnetosphere

By volume, dry air consists of 78.08 % N₂, 20.95 % O₂, 9340 ppm Ar and 420 ppm CO₂; water
vapour is typically about 1 %. At 1014 hPa surface pressure, 288 K and a mean molecular weight
of 28.97, the scale height is 8.5 km, the Bond albedo 0.294 and the geometric albedo 0.434
([NSSDC Earth Fact Sheet](quelle:nssdc-earth)); the isothermal approximation
$h_\mathrm{s} = k_\mathrm{B} T/(\bar{m}\,g)$ gives 8.4 km. The northern and southern
hemispheres reflect the same amount of sunlight to within about 0.2 W m⁻², because clouds in the
south offset the brighter land in the north, and the reflected mean varies from year to year by
only about 0.2 % ([Stephens et al. 2015](literatur:stephens-2015)). Nevertheless, the planetary
albedo reached a record low in 2023, mainly through less low cloud in the northern mid-latitudes
and the tropics. The authors see this as the main factor behind the roughly 0.2 K by which
earlier estimates of the known drivers fell short of the temperature rise
...
For 2025.0 these are $B_0 = 29733\,\mathrm{nT}$ and $m = 7.69 \cdot 10^{22}\,\mathrm{A}\,\mathrm{m}^2$;
the dipole axis is tilted by 9.2° from the rotation axis, and the geomagnetic north pole lies at
80.8° N, 72.8° W. In 1900 the moment was $8.32 \cdot 10^{22}\,\mathrm{A}\,\mathrm{m}^2$, so it
fell by 7.6 % in 125 years, and the forecast change of $g_1^0$ by +12.6 nT per year continues
the decline. In the field changes at the core–mantle boundary on sub-decadal time scales, the
CHAOS-8 model finds westward-moving features and, tentatively, small-scale eastward-moving ones
at low latitudes, at about 200 km per year, interpreted as evidence of hydromagnetic waves
...
the tidal value corresponds to 2.40 ms per century, in agreement with the lunar value. Since
1972 the core has contributed as well: once the effect of ice melt in Greenland and Antarctica is
removed, its angular velocity decreases at a constant rate and increases that of the rest of the
Earth. The extrapolation in 2024 found that UTC would need a negative leap second by 2029, and
three years earlier without the accelerated ice melt ([Agnew 2024](literatur:agnew-2024)).

## Formation and evolution

Calcium–aluminium-rich inclusions in meteorites formed
$4567.30 \pm 0.16$ million years ago ([Connelly et al. 2012](literatur:connelly-2012)). The
...
How similar the oxygen isotopes really are is disputed. Herwartz et al. measured a difference
in $\Delta^{17}\mathrm{O}$ of 12 ± 3 ppm as a trace of the impactor Theia, or alternatively of a
late addition of carbonaceous chondrites ([Herwartz et al. 2014](literatur:herwartz-2014)).
Young et al. found −1 ± 5 ppm (2 standard errors) and infer vigorous mixing in an energetic
impact ([Young et al. 2016](literatur:young-2016)). According to Cano et al., the values
...
The age of the Moon is also disputed. Zircons from Apollo 14 samples indicate a differentiated
lunar crust 4.51 billion years ago, i.e. formation of the Moon within the first 60 million years
or so ([Barboni et al. 2017](literatur:barboni-2017)). A magma ocean that takes 150 to 200
million years to solidify, combined with sample ages, leads instead to
$4.425 \pm 0.025$ billion years, equal to the uranium–lead age of the Earth, which would then
date the last core formation ([Maurice et al. 2020](literatur:maurice-2020)). A zircon whose age
of 4.4 billion years is confirmed by atom-probe tomography requires any mixing of the silicate
Earth to have happened earlier ([Valley et al. 2014](literatur:valley-2014)).

## Open questions

- **Age of the inner core:** estimates range from about 0.5 to more than 2.5 billion years,
  depending on thermal conductivity and the interpretation of the palaeofield
  ([Bono et al. 2019](literatur:bono-2019)).
- **Light elements:** tighter limits require the core temperature and a link between solid and
  liquid core compositions ([Hirose et al. 2021](literatur:hirose-2021)).
- **Rotation of the inner core:** repeated seismic waves show a pause in differential rotation
  over the past decade, perhaps part of an oscillation of about seven decades
  ([Yang and Song 2023](literatur:yang-2023)). Another analysis sees super-rotation from 2003 to
  2008, then sub-rotation two to three times slower until 2023, and calls for new coupling
  models ([Wang et al. 2024](literatur:wang-2024)).
...
- **Orbit:** elements from JPL's Table 1 (1800 to 2050) relative to the J2000 ecliptic with
  linear rates, valid for the Earth–Moon barycentre, with nominal errors of 20″ in longitude,
  8″ in latitude and 6000 km in distance ([JPL Approximate Positions](quelle:jpl-approx-pos)).
  ...
  $\mu/(1 + \mu)$ of the lunar distance, about 4670 km with the mass ratio above, or 6.4″ as
  seen from the Sun; the vector from Earth to Moon is not affected.
...
- **Data block:** the diameter of 12,742 km is twice the mean radius, not the equatorial
  diameter; the sphere is not flattened. The data block computes the orbital period of
  365.2 days with Kepler's law from $a$ at epoch and $G\,(M_\odot + M_\oplus)$: 365.2495 days,
  about ten minutes less than the $360^\circ/\dot{L} = 365.2564$ days with which the Earth is
  moved. The eccentricity 0.017 is the table value at the clock time.
...
- **Albedo:** the cloud-free day map has a mean linear reflectance of about 0.13; a factor of
  about 3.2 raises it to the geometric albedo of 0.434, so the brightness of the clouds is spread
  over land and sea. Clouds and atmosphere are missing; only the umbra is tinted for lunar
  eclipses.
