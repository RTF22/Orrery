...
**Testzahlen:** Ausgangsstand `master` nach den Nachträgen vom 19.09.2026 (Mondraten, Flug M-3 und M-4): 3839 Tests. Im Dateitest `src/data/texte/dateien.test.ts` erzeugt ein Hochschultext 10 Fälle (Thema) beziehungsweise 11 Fälle (Körper, Szene: zusätzlich „folgt der Gliederung seiner Art"). Soll: 3839 + 1 (Task 1) + 2 × 11 (Sonne) + 2 × 11 (Mond) + 2 × 10 (gebundene Rotation) + 2 × 10 (Finsternisse) + 2 × 11 (Mondtanz) + 2 × 11 (Erdaufgang) = **3968**, zuzüglich Tests aus Zwischen-Tasks für neue TeX-Befehle. Der Bericht jedes Tasks nennt die tatsächliche Gesamtzahl nach `npm test`. Weicht sie ab, die Ursache nennen, nicht den Test anpassen.
...
`npm test` → 3840. Danach Trailer- und Wortkontrolle aus der lokalen Projektanleitung.
...
- `## Im Modell` (Ruling 3, nur beschreiben, am Code nachrechnen):
  - Bahn: feste a, e, i gegen die Ekliptik J2000, Winkel linear fortgeschrieben; Knoten- und Perigäumsrate nach Meeus, um die allgemeine Präzession vermindert (seit Commit 43f34d1; `thema-bezugssysteme`, Im Modell, Punkt „Mondknoten“); keine periodischen Störungen, der Fehler in Länge in der Größenordnung der Evektionsamplitude (belegt); der Mond läuft um den Erdmittelpunkt, der auf dem Erde-Mond-Schwerpunkt sitzt (Pilot `objekt-earth`, Im Modell).
...
- Jeder Umsetzer bekommt die Gemeinsamen Vorgaben, seinen Task und die Globalen Randbedingungen wörtlich; die Modellzahlen des Mondes aus Task 3 (Nullmeridian, Pol) gibt der Controller an Task 4 bis 6 weiter.
...
8. **Ruling:** Modellzahlen, die schon in einem fachgeprüften Text stehen (etwa Pilot `szene-mondfinsternis`: 135 von 143 Kernschattenfinsternissen, drei abweichende Arten; Pilot `objekt-earth`: Globus um 75,4° verdreht am 17. September 2026), übernehmen die neuen Texte gleichlautend. Ergibt die eigene Nachrechnung einen anderen Wert, meldet der Umsetzer das im Bericht und im Ledger; der fachgeprüfte Text wird nicht in dieser Etappe geändert, die Frage geht an Jens (§8).
